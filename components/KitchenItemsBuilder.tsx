...
export function KitchenItemsBuilder() {
  ...
  // Longueur linéaire des meubles bas
  ...
  // Calcul du résultat en live via useMemo
  const result = useMemo(() => {
    ...
    return computeFromBuilderItems(payload);
  }, [items]);
  ...
  const handleSendToGoogleSheet = async () => {
    ...
    const FURNITURE_MARGIN_RATE = 0.30;
    ...
    if (worktopTotalHT > 0 && totalBaseLengthM > 0) {
      ...
      rowsMeubles.push([
        designationPlan,
        1,
        parseFloat(worktopTotalHT.toFixed(2)),
      ]);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        value={item.widthCm ?? ""}
                        onChange={(e) =>
                          handleWidthChange(item.id, e.target.value)
                        }
                      >
          ...
          {items.length > 0 && (
            <div className="flex justify-end">
              <button
                type="button"
                onClick={handleAddItem}
                className="mt-2 rounded-full bg-sky-600 hover:bg-sky-500 text-white text-xl w-10 h-10 flex items-center justify-center shadow-md"
              >
                +
              </button>
            </div>
        ...
        {/* Colonne droite : récap / chiffrage toujours visible, sans scroll interne */}
        <div className="lg:w-1/2">
          <div className="mt-4 lg:mt-0 rounded-xl border border-slate-700 bg-slate-900/80 p-4 text-sm space-y-3">
            <h3 className="text-base font-semibold text-slate-50 mb-2 text-center">
              Détail par élément
            </h3>
            ...
            <div className="space-y-2">
              {result.detailed.length === 0 && (
                <p className="text-xs text-slate-400">
                  Ajoutez des éléments à gauche pour voir le détail ici.
                </p>
              )}
              ...
              {result.detailed.map((row, idx) => {
                ...
                if (!row.price) {
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-4 text-xs">
                          ...
                          {worktopSurfaceM2 > 0 && (
                            <span>
                              Surface estimée :{" "}
                              {worktopSurfaceM2.toFixed(2)} m²
                            </span>
                          )}
                          <span className="font-semibold text-slate-50">
                            Total plan de travail :{" "}
                  ...
                  return (
                    ...
                    >
                      Élément {idx + 1} : configuration incomplète (non
                      chiffré).
                    </div>
                  );
                }
            ...
            {/* Totaux toujours affichés, même à 0 */}
            <div className="pt-2 border-t border-slate-700/60 mt-2 space-y-1">
              ...
              <p className="text-slate-200">
                Coût pose (HT) :{" "}
                {result.totals.installationCost.toFixed(2)} €
              </p>
              ...
              {(() => {
                ...
                const totalTTCAvecPlan =
                  totalHTAvecMargeEtPlan + tvaAvecPlan;
                ...
                if (worktopTotalHT > 0) {
                  return (
                    <>
                      ...
                      <p className="text-slate-200">
                        TVA (
                        {(result.totals.vatRate * 100).toFixed(0)} %) :{" "}
